...
namespace RidgeRacerVTool.RR5_Lib
{
    class Header
    {
        /// <summary>
    ...
    class RR5_LZSS
    {
        ...
        public bool Decompress(string path)
        {
            ...
            {
                // D&D file
                using (FileStream srcFileStream = new FileStream(path, FileMode.Open, FileAccess.Read))
                {
                    ...
                    using (FileStream destFileStream = new FileStream($@"{fileDirectory}\{fileName}_decomp.bin", FileMode.Create, FileAccess.ReadWrite))
                    {
